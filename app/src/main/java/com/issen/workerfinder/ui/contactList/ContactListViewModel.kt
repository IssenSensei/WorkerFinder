...
class ContactListViewModel(private val userRepository: UserRepository) : ViewModel() {
    ...
    init {
        source = userRepository.getUserContacts(currentLoggedInUserFull!!.userData.userId)
    ...
    private fun setQuerySource(selectedFilterContainer: FilterContainer): SimpleSQLiteQuery {
        var queryString = "SELECT * FROM user_table WHERE userId <> ? and userId IN ( SELECT contactId from contact_table where userId = ?) OR userId in " +
                "(SELECT userId from contact_table where contactId = ?)";
