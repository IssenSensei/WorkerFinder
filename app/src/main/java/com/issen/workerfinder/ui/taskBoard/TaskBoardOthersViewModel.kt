...
class TaskBoardOthersViewModel(private val taskRepository: TaskRepository, private val dashboardNotificationRepository: DashboardNotificationRepository) : ViewModel() {

    var source: LiveData<List<TaskModelFull>>
    val mediatorLiveData: MediatorLiveData<List<TaskModelFull>> = MediatorLiveData()

    init {
        source = taskRepository.getOthersBoardTasks(currentLoggedInUserFull!!.userData.userId)
    ...
    private fun setQuerySource(selectedFilterContainer: FilterContainer): SimpleSQLiteQuery {
        var queryString = "SELECT * FROM task_table WHERE task_worker_id = '' AND task_completion_type = 'BOARD' AND task_user_id <> ?";
    ...
    fun applyForBoardTask(taskModelFull: TaskModelFull) {
        viewModelScope.launch {
            dashboardNotificationRepository.notify(
                DashboardNotification(
                    0,
                    Date().toString(),
                    taskModelFull.task.userFirebaseKey,
                    currentLoggedInUserFull!!.userData.userId,
                    DashboardNotificationTypes.TASKBOARDAPPLIED.toString(),
                    taskModelFull.task.taskId,
                    false
                )
            )
        }
