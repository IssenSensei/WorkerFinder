...
class TaskBoardMineViewModel(private val taskRepository: TaskRepository) : ViewModel() {

    var source: LiveData<List<TaskModelFull>>
    val mediatorLiveData: MediatorLiveData<List<TaskModelFull>> = MediatorLiveData()
    init {
        source = taskRepository.getMineBoardTasks(currentLoggedInUserFull!!.userData.userId)
